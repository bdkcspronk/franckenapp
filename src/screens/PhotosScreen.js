...
export default function PhotosScreen() {
  ...
  return (
    <ScrollView style={{ flex:1, padding:16 }}>
      ...
      <Text style={{ ...theme.typography.body, marginTop: 8 }}>Albums from past events (placeholder).</Text>

      <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 12, justifyContent: 'space-between' }}>
        {placeholder.map((uri, idx) => (
          <Image key={idx} source={{ uri }} style={{ width, height: 120, marginBottom: 12, borderRadius: 8 }} />
        ))}
      </View>
    </ScrollView>
  );
